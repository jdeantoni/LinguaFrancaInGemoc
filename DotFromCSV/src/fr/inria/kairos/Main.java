...
public class Main {
            ...
            while(currentTransition != null){
                ...
                if(currentTransition.getPreviousTransition() != null)
                    sb.append(nodeId - 1).append(" -> ").append(nodeId).append(" [label=\"")
                            .append(generateLabel(currentTransition.getPreviousStepReactions())).append("\"]\n");
        ...
        for (TraceRecord record: traceRecords) {
            ...
            if(record.getEvent().equals(REACTION_STARTS)){
                if(record.getElapsedLogicalTime() == nextLogicalTimeAdvance && currentMicroStep ==
                        record.getMicroStep()){
                    if (triggersPrecededByEffects(record.getTriggeredBy(), pendingEffects)) {  // causality from previous reaction on same Logical time
                        currentTransition = addTransition(currentTransition, pendingLabels, nextLogicalTimeAdvance,
                                currentMicroStep, false);
                        if (pendingEffects.size() > 0)
                            currentTransition.setPendingEffects(pendingEffects);
                        pendingLabels = new LinkedList<>();
                    }
                ...
                if(record.getEffects() != null)
                    addReactionEffects(record.getEffects(), pendingEffects);
        }
    ...
    private static Transition addTransition(Transition currentTransition, List<String> pendingReactions,
        ...
        if(resultsFromWorkerAdvancing) {
            nextTransition.setPreviousStepReactions(new LinkedList<>());
            nextTransition.getPreviousStepReactions().add(TIME_ADVANCE_LABEL_PREFIX);
        }
        else
            nextTransition.setPreviousStepReactions(pendingReactions);
     ...
     */
    private static boolean triggersPrecededByEffects(String[] reactionTriggers, Set<String> pendingEffects) {
        if(reactionTriggers != null)
            for (String trigger: reactionTriggers) {
                if(pendingEffects.remove(trigger))
                    return true;
            }
        return false;
    }
     ...
     * @param pendingEffects set of reactions pending effects
     */
    private static void addReactionEffects(String[] effects, Set<String> pendingEffects) {
        if(effects != null)
